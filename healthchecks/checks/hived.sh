...
WGET_RESULT=$(wget -q -O - --post-data '{"jsonrpc": "2.0", "id": 1, "method": "condenser_api.get_dynamic_global_properties", "params": []}' http://haf:8091/)
if [ $? -ne 0 ]; then
...
if echo "$WGET_RESULT" | grep -q '"error":'; then
  echo "down #get_dgp() call returned an error"
  exit 2
fi

HIVED_HEAD_BLOCK_TIME_STRING=$(echo "$WGET_RESULT" | sed 's/^.*"time":"\([^"]*\)".*/\1/g')
