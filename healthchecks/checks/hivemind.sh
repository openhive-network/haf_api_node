...
HIVEMIND_ADJUSTED_AGE=$(adjust_age_for_ci "$HIVEMIND_LAST_IMPORTED_BLOCK_AGE")
if [ "$HIVEMIND_ADJUSTED_AGE" -gt 60 ]; then
  age_string=$(format_seconds "$HIVEMIND_LAST_IMPORTED_BLOCK_AGE")
  if [ "$TIME_OFFSET" -gt 0 ]; then
    echo "down #hivemind block over a minute old ($age_string, adjusted from CI offset)"
  else
