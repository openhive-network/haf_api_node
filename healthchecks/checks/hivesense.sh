...
HIVESENSE_LAST_PROCESSED_BLOCK_AGE=$(psql "$POSTGRES_URL_HIVESENSE" --quiet --no-align --tuples-only --command="select extract('epoch' from hive.get_app_current_block_age('hivesense_app1'))::integer")
if [ "$HIVESENSE_LAST_PROCESSED_BLOCK_AGE" -gt 60 ]; then
