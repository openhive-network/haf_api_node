...
# count=0
# until [[ $(docker exec --env LC_ALL="C" haf-world-reputation-tracker-block-processing-1 psql "${HAF_DB_URL_REPTRACKER:?}" --quiet --tuples-only --no-align --command "${PSQL_COMMAND_REPTRACKER:?}") == "${LAST_BLOCK_NUMBER:?}" ]]
# do
#     CURRENT_BLOCK=$(docker exec --env LC_ALL="C" haf-world-reputation-tracker-block-processing-1 psql "${HAF_DB_URL_REPTRACKER:?}" --quiet --tuples-only --no-align --command "${PSQL_COMMAND_REPTRACKER:?}")
#     echo -e "Waiting for Reputation Tracker replay to finish...\n Current block: ${CURRENT_BLOCK:?}"
#     count=$((count+10))
#     [[ $count -eq "${REPLAY_TIMEOUT:-6000}" ]] && exit 1
#     sleep 10s
# done
...
# count=0
# until [[ $(docker exec --env LC_ALL="C" haf-world-hivemind-block-processing-1 psql "${HAF_DB_URL_HIVEMIND:?}" --quiet --tuples-only --no-align --command "${PSQL_COMMAND_HIVEMIND:?}") == "${LAST_BLOCK_NUMBER:?}" ]]
# do
#     CURRENT_BLOCK=$(docker exec --env LC_ALL="C" haf-world-hivemind-block-processing-1 psql "${HAF_DB_URL_HIVEMIND:?}" --quiet --tuples-only --no-align --command "${PSQL_COMMAND_HIVEMIND:?}")
